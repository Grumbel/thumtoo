...
3. **Store-only (`THUMTOO_STORE_ONLY=1`, ≥247–248):** no legacy Database/BlobStore.
   Durable Store only. Plain `file://` images: Store-first probe, session pixels,
   and tile encode (single cell + pyramid) written to Store bulk.
4. Expand Store-only to archives/PDF; remove dual-write helpers.
...
| Store-only (`THUMTOO_STORE_ONLY`) | **Experimental** (≥247–248; file probe/pixels/tiles) |
