...
3. **Store-only (`THUMTOO_STORE_ONLY=1`, ≥247):** no legacy Database/BlobStore open.
   Durable Store only under the cache. Plain `file://` images: Store-first probe
   + session EnsurePixels. Tile encode and non-image URIs still need dual-path.
4. Remove dual-write helpers; expand Store-only to archives/PDF/tiles.
...
| Store-only (`THUMTOO_STORE_ONLY`) | **Experimental** (≥247; no legacy open; file images) |
