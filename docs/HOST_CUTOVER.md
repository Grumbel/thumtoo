...
3. **Store-only (`THUMTOO_STORE_ONLY=1`, ≥247–249):** no legacy Database/BlobStore.
   Durable Store only. Covered: plain `file://` images, PDF pages, archive members
   (probe, session pixels, tile cells). DjVu/EPUB/HTTP still dual-path.
4. Remove dual-write helpers; DjVu/EPUB Store-only parity.
...
| Store-only (`THUMTOO_STORE_ONLY`) | **Experimental** (≥247–249; file/PDF/archive) |
